...
pub(super) fn fmt_capacity(capacity: f32) -> String {
...
pub(super) fn get_value(device: &str, name: &str) -> Result<i32> {
    io::read_int_from_file(&format!("{}/{}/{}", POWER_SUPPLY_PATH, device, name))
        .wrap_error(FEATURE_NAME, &format!("error reading {}/{}", device, name))
}

pub(super) fn get_value2(device: &str, name1: &str, name2: &str) -> Result<i32> {
